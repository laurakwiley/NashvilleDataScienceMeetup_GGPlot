...
mitodata <- read.table(file="Example_SolarPlot_Data.txt", header=T) %>% tbl_df()
...
ggplot(mitodata, aes(x = bp,y = neglogp,color = gene)) +
  ...
  geom_text(data = bdries, aes(x,y,label=x),size = 2.5)

ggsave("Example_SolarPlot_Plot.png", w=6, h=6, dpi=110)
